# modified from script authored by Raul Aguirre Gamboa
...
test <- readRDS("fgsea_ora_reactome_top10perTreatment_new.rds")
...
NES_mat <- gsea_res %>%
  ...
  select(stims,padj,gs_description) %>%
  spread(gs_description,padj)
...
go_bp_plot <- go_bp_dat %>%
  ...
  ungroup() %>%
  drop_na() %>%
  mutate(stims=factor(stims, levels=c("fluni", "hc", "myr","fen", "nerol", "5f", "hq", "bg"))) %>%
  ggplot(aes(y=label, x=stims))+
  geom_point(aes(size=-log10(padj), fill=-log10(padj), color=padj<0.1), shape=21, stroke=1) +
