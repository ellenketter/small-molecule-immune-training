...
da_myr <- results_myr[which(results_myr$DA != "NO"),]
da_myr <- da_myr %>%
   select(genes, logFC)
da_myr$peakID <- rownames(da_myr)
...
allLogFC <- merge(da_myr, allLogFC, by= 'genes')
allLogFC_names <- c("genes","myr","peakID", names(mylist))
...
for (i in 1:length(names(mylist))){
   ...
   i_file_name <- paste0("beta_correlation_myrDA_",names(mylist)[i],"_logFC.tiff")

   p <- allLogFC %>%  
      ggplot(aes(x=.data[["myr"]], y= .data[[names(mylist)[i]]])) + 
      geom_point(shape=21, color="black", size=0.1) + 
      theme_classic() + 
      labs(x="Myricetin LogFC", y=paste0(names(mylist)[i], " LogFC"), fill="")+
