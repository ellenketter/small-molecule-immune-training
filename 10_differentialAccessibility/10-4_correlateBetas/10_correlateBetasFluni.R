...
treatments <- list(results_5f=results_5f,results_bg=results_bg, results_fen=results_fen,
                   ...
                   results_hc=results_hc, results_hq=results_hq, results_myr=results_myr,
                   results_nerol=results_nerol)

# confirm concordance between gene names, should print "integer(0)" for all
for (i in 1:length(treatments)){
   not <- which(results_fluni$genes != treatments[i]$genes)
   print(not)
}

da_fluni <- results_fluni[which(results_fluni$DA != "NO"),]
da_fluni <- da_fluni %>%
   select(genes, logFC)
da_fluni$peakID <- rownames(da_fluni)
...
allLogFC <- merge(da_fluni, allLogFC, by= 'genes')
allLogFC_names <- c("genes","fluni","peakID", names(mylist))
...
for (i in 1:length(names(mylist))){
   ...
   i_file_name <- paste0("beta_correlation_fluniDA_",names(mylist)[i],"_logFC.tiff")

   p <- allLogFC %>%  
      ggplot(aes(x=.data[["fluni"]], y= .data[[names(mylist)[i]]])) + 
      geom_point(shape=21, color="black", size=0.1) + 
      theme_classic() + 
      labs(x="Flunisolide LogFC", y=paste0(names(mylist)[i], " LogFC"), fill="")+
