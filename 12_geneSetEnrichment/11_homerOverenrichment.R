...
OUT_DIR <- "11_homerOverEnrichment/"
# dir.create(OUT_DIR)


# Reactome pathways -------------------------------------------------------

molecules <- c("bg","fluni","myr","hc","fen")
sig_pathways <- vector()
for(i in 1:length(molecules)){
  ...
  results_sub1 <- subset(results, results$q < 0.05)[1:15,]
...
results_list <- list(bg_results,fluni_results, myr_results, hc_results, fen_results) 
...
tiff(paste0(OUT_DIR,"enriched_reactome_pathways_q0.05.tiff"), units="in", width=8, height=6, res=400)
ggplot(df, aes(x = clust, y = path)) +
  geom_point(aes(size = -log10(pval), fill = g), alpha = 0.75, shape = 21) + theme_classic() + scale_fill_manual(values=c("white", "#D82632"))+
  scale_size_continuous(limits = c(0, 10)) +
  labs( x= "", y = "", size = "Significance", fill = "Significance q <= 0.05") + theme(axis.text.y = element_text(size=6))+
