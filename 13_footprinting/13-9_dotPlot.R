...
setwd("~/Downloads/")
new <- readRDS("mergedStatsPadj.rds")
replicates <- readRDS("diffFootprinting/mergedStats.rds")
new %>%
...
NES_mat <- new %>%
  ...
  select(molecule,Motif,foldChange) %>%
  spread(Motif,foldChange)
...
go_bp_plot <- go_bp_dat %>%
  ...
  geom_point(aes(size=-log10(pAdj), fill=log2(foldChange), color=pAdj<0.1), shape=21, stroke=1) +
  ...
  scale_color_manual(values = c("lightgray","#53565A")) +
                                 ...
                                 # DEG_res_bar
                                 pre_tt_gsea_dot_plot_file <- "TF_dot_plot_pAdj-top5.pdf"
