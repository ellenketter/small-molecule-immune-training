...
meta_data <- read.table(file="../../../analysis/10_differentialAccessibility/meta_data.txt", header=TRUE)
...
design <- model.matrix(~1 + molecule, data=meta_data)
...
saveRDS(fit, file = paste0(OUT_DIR, "fit_allPBS.rds"))

expression <- v2$E  
write.table(expression, file=paste0("../10_differentialAccessibility/expressionCPMs_allPBS.txt"), quote = FALSE, sep = ",") 
...
tiff(paste0(OUT_DIR,"PCA_PBS1-6_noOutliers.tiff"), units="in", width=4, height=3, res=350)
