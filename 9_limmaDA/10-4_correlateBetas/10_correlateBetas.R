...
# Correlate betas ---------------------------------------------------------
treatments <- list(results_5f=results_5f, results_fen=results_fen,
                   results_fluni= results_fluni, results_hc=results_hc, 
...
for (i in 1:length(treatments)){
   not <- which(results_bg$genes != treatments[i]$genes)
   print(not)
}

da_bg <- results_bg[which(results_bg$DA != "NO"),]
da_bg <- da_bg %>%
   select(genes, logFC)
da_bg$peakID <- rownames(da_bg)
...
allLogFC <- merge(da_bg, allLogFC, by= 'genes')
allLogFC_names <- c("genes","bg","peakID", names(mylist))
...
for (i in 1:length(names(mylist))){
   ...
   i_file_name <- paste0("beta_correlation_bgDA_",names(mylist)[i],"_logFC.tiff")

   p <- allLogFC %>%  
      ggplot(aes(x=.data[["bg"]], y= .data[[names(mylist)[i]]])) + 
      geom_point(shape=21, color="black", size=0.1) + 
      theme_classic() + 
      labs(x="Beta Glucan LogFC", y=paste0(names(mylist)[i], " LogFC"), fill="")+
